...
exports.SuggestionsMentorsGet = async (req, res) => {
  ...
  try {
    let sgs = await Mentor.find({
      $or: [
        { lookingfor_availabilty: mentee.lookingfor_availabilty },
        { lookingfor_skillLevel: mentee.lookingfor_skillLevel },
        { lookingfor_ethnicity: mentee.lookingfor_ethnicity },
        { lookingfor_education: mentee.lookingfor_education },
      ],
    }).populate("user_id", "-password");

    return successWithData(res, OK, { sgs, mentorshipInfo: mentee });
  } catch (error) {
    return AppError.tryCatchError(res, error);
  }
};
...
exports.SuggestionsMentesGet = async (req, res) => {
  ...
  try {
    let sgs = await Mentee.find({
      $or: [
        { lookingfor_availabilty: mentor.lookingfor_availabilty },
        { lookingfor_skillLevel: mentor.lookingfor_skillLevel },
        { lookingfor_ethnicity: mentor.lookingfor_ethnicity },
        { lookingfor_education: mentor.lookingfor_education },
      ],
    }).populate("user_id", "-password");

    return successWithData(res, OK, { sgs, mentorshipInfo: mentor });
  } catch (error) {
    return AppError.tryCatchError(res, error);
  }
};
...
exports.SuggestionsCoursesGet = async (req, res) => {
  ...
  const SgsIntersts = Object.values(
    user.passions,
...
exports.SuggestionsCompaniesGet = async (req, res) => {
  ...
  try {
    const sgsCompanies = await Company.aggregate([
      { $unwind: "$specialties" },
      { $match: { specialties: { $in: [...sgsIntersts] } } },
    ]);
