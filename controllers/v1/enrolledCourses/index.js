...
exports.createEnrolledCourse = async (req, res, next) => {
  ...
  try {
    ...
    const findCourse = await Course.findById(req.body.courseId).populate("_id");
    if (!findCourse) {
    ...
    const courseReputation = await Reputations.findOne({
      courseEnrollCount: 1
    });
    ...
    if (!(courseReputation && userReputation)) {
...
exports.getAllEnrolledCourse = async (req, res, next) => {
  try {
    ...
    return successWithData(
      res,
      OK,
      "EnrolledCourses fetched successfully",
      EnrolledCourses
    );
  } catch (err) {
    console.log(err);
...
exports.getUserEnrolledCourse = async (req, res, next) => {
  try {
    const EnrolledCourses = await EnrolledCourse.find({ user_id: req.user.id })
      .populate("user_id")
      .populate("courseId")
      .sort("-createdAt");
    if (!EnrolledCourses) {
      let error = { message: "you have no courses" };
...
exports.getEnrolledCourse = async (req, res, next) => {
  try {
    const EnrolledCourses = await EnrolledCourse.findById(req.params.id)
      ...
      .sort("-createdAt");
    if (!EnrolledCourses) {
      let error = { message: "undefined enrolled Course" };
      return AppError.tryCatchError(res, error);
    }
    return successWithData(
      res,
      OK,
      "EnrolledCourse fetched successfully",
      EnrolledCourses
    );
  } catch (err) {
    console.log(err);
...
exports.updateEnrolledCourse = async (req, res, next) => {
  try {
    ...
    return successWithData(
      res,
      OK,
      "EnrolledCourse modified",
      modifiedEnrolledCourse
    );
  } catch (err) {
    console.log(err);
    return AppError.tryCatchError(res, err);
