...
exports.createUpVotes = async (req, res, next) => {
  try {
    ...
    const voted = await UpVotes.find({}).sort("-createdAt");
...
if(req.body.proposal_Id){
  ...
  console.log(voted)
    ...
    voted.map((v) => {
      if ((v.proposal_Id ? v.proposal_Id : '').toString() === (req.body.proposal_Id).toString() && (v.authorId).toString() === (req.user.id).toString()){
      let errors = "Already voted";
      return AppError.validationError(res, UNAUTHORIZED, errors);
      }
    })
...
exports.getStartupUpVotes = async (req, res, next) => {
    try {
      const UpVotess = await UpVotes.find({startup_Id : req.params.id}).populate("authorId").populate("proposal_Id").populate("startup_Id").sort("-createdAt");
      if (!UpVotess) { let error = {message: "undefined Upvote"}; return AppError.tryCatchError(res, error);}
      return successWithData(res, OK, "UpVotes fetched successfully", UpVotess);
    } catch (err) {
      console.log(err);
  ...
  exports.getProposalUpVotes = async (req, res, next) => {
    try {
      const UpVotess = await UpVotes.find({proposal_Id : req.params.id}).populate("authorId").populate("proposal_Id").populate("startup_Id").sort("-createdAt");
      if (!UpVotess) { let error = {message: "undefined Upvote"}; return AppError.tryCatchError(res, error);}
      return successWithData(res, OK, "UpVotes fetched successfully", UpVotess);
    } catch (err) {
      console.log(err);
