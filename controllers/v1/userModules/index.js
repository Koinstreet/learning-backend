...
exports.createUserModule = async (req, res, next) => {
  ...
  try {
    ...
    const findModule = await Module.findById(req.body.moduleId).populate("_id");
    if (!findModule) {
    ...
    const UserModuless = await UserModules.find({ user_id: req.user.id }).sort(
      "-createdAt"
    );
...
exports.getAllUserModules = async (req, res, next) => {
  try {
    const UserModuless = await UserModules.find({})
      .populate("user_id")
      .populate("moduleId")
      .sort("-createdAt");
    return successWithData(
      res,
      OK,
      "UserModuless fetched successfully",
      UserModuless
    );
  } catch (err) {
    console.log(err);
...
exports.getUserUserModules = async (req, res, next) => {
  try {
    const course = await Course.findById(req.params.courseId);
    if (!course) {
      let error = { message: "undefined course" };
      return AppError.tryCatchError(res, error);
    }
    const UserModuless = await UserModules.find({ user_id: req.user.id })
      .populate("user_id")
      .populate("moduleId")
      .sort("-createdAt");
    if (!UserModuless) {
      let error = { message: "you have no Modules" };
      return AppError.tryCatchError(res, error);
    }
    return successWithData(
      res,
...
exports.getModuleById = async (req, res, next) => {
  try {
    ...
    const UserModuless = await UserModules.find({ user_id: req.params.id })
      .populate("user_id")
      .populate("moduleId")
      .sort("-createdAt");
    if (!UserModuless) {
...
exports.getUserModule = async (req, res, next) => {
  try {
    ...
    const UserModuless = await UserModules.findById(req.params.id)
      .populate("user_id")
      .populate("moduleId")
      .sort("-createdAt");
    if (!UserModuless) {
      let error = { message: "undefined user Module" };
      return AppError.tryCatchError(res, error);
    }
    return successWithData(
      res,
...
exports.updateUserModules = async (req, res, next) => {
  try {
    ...
    if (!UserModulesUpdate) {
      let error = { message: "undefined enrolled Module" };
      return AppError.tryCatchError(res, error);
    }
