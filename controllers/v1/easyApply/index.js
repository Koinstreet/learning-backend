...
exports.createEasyApply = async (req, res, next) => {
  ...
  try {
    ...
    const easyApplied = await EasyApply.find({ authorId: req.user.id }).sort(
      "-createdAt"
    );
    ...
    const applierLocation = newEasyApply.country
      ? newEasyApply.country
      : "" + newEasyApply.city
      ? newEasyApply.city
      : "";
    ...
    sendEmail(
      emailTemplateAuthor(
        findJob.authorId.firstName,
        newEasyApply.email ? newEasyApply.email : req.user.email,
        applierName,
        applierLocation,
        newEasyApply.relevant_experience,
        newEasyApply.company,
        findJob.job_title,
        findJob.job_description,
        newEasyApply.resume,
        newEasyApply.coverLetter
      ),
      subjectAuthor,
      findJob.authorId.email
    );
    sendEmail(
      emailTemplateUser(
        req.user.firstName,
    ...
    const jobReputation = await Reputations.findOne({
      jobApplyCount: 1
    });
    ...
    if (!(jobReputation && userReputation)) {
      const newReputation = await Reputations.create({
        userId: req.user.id,
        title: "Applied to your first Job",
        job_id: req.body.job_id,
        type: "jobApplyCount",
        jobApplyCount: 1
      });
...
exports.getAllEasyApplied = async (req, res, next) => {
  try {
    const easyApplied = await EasyApply.find({})
      .populate("authorId")
      .populate("job_id")
      .sort("-createdAt");
    return successWithData(
      res,
      OK,
      "EasyApplied fetched successfully",
      easyApplied
    );
  } catch (err) {
    console.log(err);
...
exports.getAllUserEasyApplies = async (req, res, next) => {
  try {
    const EasyApplys = await EasyApply.find({ authorId: req.user.id })
      .populate("authorId")
      .populate("job_id")
      .sort("-createdAt");
    if (!EasyApplys) {
      let error = {
        message: "you have not applied to any job using Easy apply method"
      };
    ...
    return successWithData(
      res,
      OK,
      "user easy applied fetched successfully",
      EasyApplys
    );
  } catch (err) {
    console.log(err);
...
exports.getEasyApply = async (req, res, next) => {
  try {
    const easyApply = await EasyApply.findById(req.params.id)
      .populate("authorId")
      .populate("job_id")
      .sort("-createdAt");
    if (!easyApply) {
      let error = { message: "undefined easy apply" };
      return AppError.tryCatchError(res, error);
    }
    ...
    return successWithData(
      res,
      OK,
      "EasyApply fetched successfully",
      easyApply
    );
  } catch (err) {
    console.log(err);
...
exports.updateEasyApply = async (req, res, next) => {
  try {
    ...
    const appliedJob = await EasyApply.findById(req.params.id)
      .populate("authorId")
      .populate("job_id")
      .sort("-createdAt");
    if (!appliedJob) {
      let error = { message: "undefined applied job" };
      return AppError.tryCatchError(res, error);
    }

    if (appliedJob.authorId._id.toString() !== req.user.id.toString()) {
      let error = {
        message: "you don't have access to this candidate and job"
      };
      return AppError.tryCatchError(res, error);
    }
...
exports.approveCandidate = async (req, res, next) => {
  try {
    ...
    const company = await Companies.findById(
      appliedJob.job_id.companyId
    ).populate("authorId");
    if (!company) {
      let error = { message: "undefined company" };
      return AppError.tryCatchError(res, error);
    }
    ...
    sendEmail(
      emailTemplateApproved(
        appliedJob.authorId.firstName,
        appliedJob.job_id.job_title,
        company.company_name,
        req.user.email
      ),
      subjectApproved,
      appliedJob.authorId.email
    );
